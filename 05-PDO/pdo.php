...
//Ici, on modifie dans la table 'employes' la colonne 'salaire' A CONDITION que dans la colonne id_employes, ce soit égal à 991
...
//rowCount() : permet de retourner le nombre de ligne de résultat retournée par la requête

while( $employe = $pdostatement->fetch( PDO::FETCH_ASSOC ) ){
	//TANT QU'il y a une ligne de résultat retournée par le fetch()
		//Ici, une ligne correspond à un employés
...
//--------------------------------------------------------------------------------
echo "<h2> QUERY() / SELECT / fetchAll() </h2>";

$pdostatement = $pdo->query(" SELECT * FROM employes ");

$donnee = $pdostatement->fetchAll( PDO::FETCH_ASSOC );

	//fetchAll() : permet de retourner TOUTES les lignes de résultats dans un tableau multidimentionnel
	//PDO::FETCH_ASSOC : permet d'indéxer les tableaux représentants les lignes de résultats avec le nom des champs de la table

// print '<pre>';
// 	print_r( $donnee );
// print '</pre>';

foreach( $donnee as $contenu ){
...
//--------------------------------------------------------------------------------
echo "<h2> prepare() / bindParam() / execute() </h2>";
...
$nom = 'winter';
	...
	//bindParam() : reçoit exclusivement une variable en justification des marqueurs
